...
const loader=new GLTFLoader(),tiles=new Map(),pending=new Set(),failed=new Set();let manifest,overview,streets=[],cameraMarkers,ready=false,lastLoad=0,desired=new Set(),focusTile=null;
...
function updateTiles(){
 ...
 for(const t of selected){
  ...
  pending.add(t.id);
  loader.load(t.url,gltf=>{pending.delete(t.id);updateLayer(gltf.scene);gltf.scene.visible=desired.has(t.id);scene.add(gltf.scene);tiles.set(t.id,{group:gltf.scene,lastSeen:performance.now()});lastLoad=0;},undefined,error=>{pending.delete(t.id);failed.add(t.id);console.error('Tile load failed',t.id,error);lastLoad=0;});
 }
...
async function init(){
 ...
 window.cityModel={get weather(){return weather.state;},get ready(){return ready;},get loadedTiles(){return tiles.size;},get pendingTiles(){return pending.size;},get failedTiles(){return failed.size;},get focusTile(){return focusTile;},get statistics(){return manifest.statistics;},get bounds(){return manifest.boundsXZ;}};
}
...
for(const id of ['buildings','minor'])$(id).onchange=()=>{if(overview)updateLayer(overview);for(const e of tiles.values())updateLayer(e.group);};
